...
export default function IngresosPage() {
  ...
  const { data: ingresos = [], isLoading } = useQuery({
    queryKey: ['ingresos', condominioId],
  ...
  const crearMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['ingresos', condominioId] });
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-on-surface">Ingresos</h1>
      ...
      <Modal abierto={modalAbierto} onCerrar={() => setModalAbierto(false)} titulo="Registrar ingreso">
        <IngresoForm onSubmit={async (d) => crearMutation.mutateAsync(d)} cargando={crearMutation.isPending} />
      </Modal>
    </div>
  );
}